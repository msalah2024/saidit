...
export default function Page() {
  const [selectedCommunity, setSelectedCommunity] =
    useState<Tables<"communities"> | null>(null);
  ...
  const tabs = ["text", "images", "videos", "link", "poll"];
      ...
      <div className="flex flex-col gap-4 p-6 w-full max-w-4xl">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange}>
          <TabsList className="h-fit bg-background gap-2 sm:gap-6">
            {tabs.map((tab) => (
              <TabsTrigger
                disabled={tab !== "text" && tab !== "link"}
                key={tab}
                value={tab}
                className="border-b-background py-2 text-primary-foreground-muted rounded-none 
