...
export default function ChangeSocialLinks({ user, profile, isDesktop, syncedPlatforms, fetchLinks }: ChangeSocialLinksProps) {
    ...
    const normalizedSyncedPlatforms: SocialPlatformExtended[] = (syncedPlatforms ?? []).map((platform) => {
        const match = socialPlatforms.find((sp) => sp.name.toLowerCase() === platform.social_name.toLowerCase())
    ...
    const combinedPlatforms: SocialPlatformExtended[] = [...(normalizedSyncedPlatforms ?? []), ...socialPlatforms]

    const filteredPlatforms: SocialPlatformExtended[] = combinedPlatforms.filter((platform) =>
    ...
    async function onSubmit(data: z.infer<typeof SocialLinkSchema>) {
        ...
        const doesExit = combinedPlatforms.find(
            (social) => social.username === data.text && social.name === selectedPlatform?.name,
        )
        if (doesExit) {
            form.setError("text", {
                type: "custom",
                ...
                return (
                    <div className="flex flex-col justify-between h-full gap-4">
                                ...
                                {filteredPlatforms.map((link) => {
                                    return (
                                        <Button
                                            ...
                                            onClick={() => handleSocialClick(link, link.username ?? "")}
                                        >
                        ...
                        {isDesktop ? (
                            ...
                            <div className="flex flex-col gap-2 justify-end mb-4 mt-0">
                                <DialogClose asChild>
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormControl>
                                                <Input placeholder={selectedPlatform?.placeholder} {...field} type="text" className="p-6" />
                                            </FormControl>
                                ...
                                {isDesktop ? (
                                    ...
                                    <div className="flex flex-col gap-2 justify-end my-4">
                                        ...
                                        <Button type="submit" disabled={isSubmitting} className="rounded-full p-6">
                                            {isSubmitting ? (
                                                <>
                                                    <Loader2 className="mr-1 h-4 w-4 animate-spin" />
                                                    Saving...
