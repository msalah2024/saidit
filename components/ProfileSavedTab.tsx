...
function SavedPostsTab({ profile }: { profile: { account_id: string } }) {
        ...
        <div style={{ height: `${virtualizer.getTotalSize()}px`, width: "100%", position: "relative" }}>
          {virtualizer.getVirtualItems().map((virtualItem) => {
            ...
            return (
              ...
              >
                <FeedPostCard post={item} setItems={setItems} removeOnUnsave />
              </div>
            )
          })}
        </div>
...
function SavedCommentsTab({ profile }: { profile: { account_id: string } }) {
  const supabase = createClient()
  ...
  const fetchItems = async (from: number) => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        created_at,
        comments!inner(
          id, body, stripped_body, created_at, net_votes, slug, deleted,
          users(username, avatar_url, verified),
          comments_votes(vote_type, voter_id, id),
          posts(title, slug, communities(community_name, image_url))
        )
      `)
      .eq("user_id", profile.account_id)
      .order("created_at", { ascending: false })
    ...
    if (error) { console.error(error); return [] }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return (data ?? []).map((v: any) => v.comments).filter((c: any) => c && !c.deleted) as ProfileComment[]
  }
...
export default function ProfileSavedTab() {
  ...
  const { savedType } = useSavedType()

  return (
    <div className="min-h-screen border-t">
      <div className="mx-auto">
        {savedType === "posts" && <SavedPostsTab profile={profile} />}
        {savedType === "comments" && <SavedCommentsTab profile={profile} />}
      </div>
    </div>
  )
}
